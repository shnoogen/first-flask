...
def index():
    ...
    for kanye in kanye_list:
        ids_list.append(kanye['Identification'])
        name_list.append(kanye['Single'])
    ...
    return render_template('index.html', pairs=pairs_list, the_title="Kanye Single Index")
...
def detail(num):
    ...
    return render_template('kanye.html', kanye=kanye_dict, ord=ord, the_title=kanye_dict['Single'])
...
def search():
    ...
    if request.method == "POST":
        ...
        for president in kanye_list:
            if text.lower() in president[category].lower():
                ids_list.append(president['Identification'])
                name_list.append(president['Single'])
